                    ...
                    <div class="pr-5 ">
                        @foreach ($posts as $post)
                            <div class="bg-white p-5 mt-5">
                                <div class=" pb-3 flex justify-between items-baseline">
                                    <div>
                                        <h2>{{ $post->title }}</h2>
                                        <p>Postado {{ $post->created_at->format('d/m/Y') }}</p>
                                        @if ($post->updated_at != $post->created_at)
                                            <span class="text-gray-400">( Atualizado {{ $post->updated_at->format('d/m/Y') }} )</span>
                                        @endif
                                    </div>
                                    <div class="flex">
                                ...
                                <p class="py-5">{{ $post->excerpt }}</p>
                                <a href="{{ route('post', $post->slug) }}" class="text-yellow-400">Leia mais</a>
                            </div>
                        @endforeach
